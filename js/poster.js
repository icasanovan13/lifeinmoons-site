// LifeInMoons — the A3 print poster, in the browser.
//
// A faithful port of the app's Poster.swift: all-vector, filled moons for the
// life lived, outlines for the life ahead, a two-line date under every moon,
// the current moon ringed. Like the app (A3 is the poster standard, per
// docs/JOURNAL.md in the app repo), the artwork is laid out in the A2-sized
// reference space where all the spacing was tuned, then scaled ×1/√2 onto an
// A3 page — same aspect ratio, every proportion survives exactly.
// Differences from the app: no memory stars (the web knows no memories), and
// PDF core fonts — Times for the serif voice, Helvetica for the tracked caps.
//
...
const Poster = (() => {
  const pageW = 1190.55, pageH = 1683.78; // A2 reference space (420 × 594 mm)
  const K = 1 / Math.SQRT2;               // → A3 page (297 × 420 mm)
  ...
  // all coordinates below are in the A2 reference space; ×K on emission
  ...
  function hairline(doc, y) {
    ...
    doc.line(pageW * 0.3 * K, y * K, pageW * 0.7 * K, y * K);
  }

  // life: { birthday: Date, horizon, moons, current, count }
  function build(life) {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({ unit: "pt", format: [pageW * K, pageH * K], compress: true });

    doc.setFillColor(NIGHT[0], NIGHT[1], NIGHT[2]);
    doc.rect(0, 0, pageW * K, pageH * K, "F");
